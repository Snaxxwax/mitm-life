...
function validateMarkdownFiles() {
  ...
  markdownFiles.forEach(file => {
    ...
    if (content.startsWith('---')) {
      ...
      if (frontmatterEnd !== -1) {
        ...
        if (!frontmatter.includes('title:')) {
          addIssue('frontmatter', file, 'Missing title in frontmatter', 'error');
        }
        if (!frontmatter.includes('description:')) {
          addIssue('frontmatter', file, 'Missing description in frontmatter', 'error');
        }
        if (file.includes('content/') && !frontmatter.includes('pubDate:')) {
          addIssue('frontmatter', file, 'Missing pubDate in frontmatter', 'error');
        }
      }
    }
      ...
      internalLinks.forEach(link => {
        ...
        if (match && match[2].startsWith('/')) {
          ...
          if (!linkPath.startsWith('/http') && !linkPath.includes('#')) {
            addIssue('link', file, `Potential broken internal link: ${link}`, 'warn');
          }
        }
      });
    }
    ...
    if (contentWithoutFrontmatter.trim().length < 100) {
      addIssue('content', file, 'Content is very short (< 100 characters)', 'warn');
    }
  });
}
...
function validateHTMLOutput() {
  ...
  if (!fs.existsSync(distPath)) {
    addIssue('build', 'system', 'No dist folder found - run build first', 'error');
    return;
  }
  ...
  htmlFiles.slice(0, 5).forEach(file => { // Limit to first 5 files to avoid spam
    try {
...
function checkImageOptimization() {
  ...
  largeImages.forEach(img => {
    ...
    addIssue('image', img.path, `Large image file (${sizeKB}KB) - consider optimization`, 'warn');
  });
  
  console.log(`Checked ${imageFiles.length} images, found ${largeImages.length} large files`);
}
