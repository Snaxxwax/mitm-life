...
export async function getContentStats() {
  ...
  try {
    ...
    const recentContent = allContent.filter(
      item => new Date(item.data.pubDate) > weekAgo
    );

    const updatedContent = allContent.filter(
      item => item.data.updatedDate && new Date(item.data.updatedDate) > weekAgo
    );
  ...
  try {
    ...
    return categories.map(category => {
      ...
      const difficultyCount = {
        beginner: collection.filter(
          (item: ContentItem) => item.data.difficulty === 'beginner'
        ).length,
        intermediate: collection.filter(
          (item: ContentItem) => item.data.difficulty === 'intermediate'
        ).length,
        advanced: collection.filter(
          (item: ContentItem) => item.data.difficulty === 'advanced'
        ).length,
      };
      ...
      const mostRecentDate =
        sortedByDate.length > 0
          ? new Date(
              sortedByDate[0].data.updatedDate || sortedByDate[0].data.pubDate
            )
          : new Date();
