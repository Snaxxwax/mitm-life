...
function analyzeBundleSize() {
  ...
  if (totalSize > 1024 * 1024) {
    // > 1MB
    console.log(
      '⚠️  Total bundle size is over 1MB - consider optimization strategies'
    );
  }
  ...
  console.log(`\n📈 Summary:`);
  console.log(
    `   JavaScript files: ${jsFiles.length} (${(jsFiles.reduce((sum, f) => sum + f.size, 0) / 1024).toFixed(2)} KB)`
  );
  console.log(
    `   CSS files: ${cssFiles.length} (${(cssFiles.reduce((sum, f) => sum + f.size, 0) / 1024).toFixed(2)} KB)`
  );
}
